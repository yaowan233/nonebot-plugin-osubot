...
后台任务最多同时 8 个，每个任务限时 900 秒。同一 Bot、会话、发起用户的相同请求跨 AI 轮次复用，
准备中不重复提交，成功后 120 秒内不再次发图；最多保留 64 个任务记录，到期自动清理，失败可重试。
模式和目标别名、筛选列表顺序会归一化，图片分析开关不触发再次发送。修改筛选、切换用户或会话不互相抑制。
用户讨论、反馈或催促已有推荐时，工具指令明确禁止再次调用。缺失会话标识时仅在当前工具上下文去重。
空结果、失败和超时在后台通知原会话；正常结束 AI 回合不会取消已受理的推荐。
...
`tests/view_output.py` 默认用四模式各 20 张固定样例验证实际渲染并上传图片，不请求生产推荐服务。
任务提交、轮询与响应转换由独立单元测试覆盖。线上推荐与并发压力检查需显式设置
`OSUBOT_TEST_LIVE_RECOMMEND=1`，同时在私有测试环境配置支持 jobs 接口的
`OSU_RECOMMEND_API` 和 `OSU_RECOMMEND_API_TOKEN`；普通 CI 不主动给生产服务施压。
