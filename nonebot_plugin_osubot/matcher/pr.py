...
async def _recent(event: Event, state: T_State):
    ...
    if state["range"]:
        ...
        pic = await draw_pfm("relist", state["user"], scores, scores, mode, source=state["source"], is_lazer=player.lazer_mode)
...
async def _pr(event: Event, state: T_State):
    ...
    if state["range"]:
        ...
        pic = await draw_pfm("prlist", state["user"], scores, scores, mode, source=state["source"], is_lazer=player.lazer_mode)
        await UniMessage.image(raw=pic).finish(reply_to=True)
    if state["day"] == 0:
        state["day"] = 1
    try:
