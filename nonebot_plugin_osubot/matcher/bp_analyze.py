...
rank_color = {'X': '#ffe193', 'XH': '#ffe193', 'S': '#ffe193', 'SH': '#ffe193', 'A': '#88da20', 'B': '#e9b941', 'C': '#fa8a59', 'D': '#f55757'}
...
async def _(state: T_State):
    ...
    ax2 = ax1.twinx()
    # 绘制柱状图
    ax1.bar(range(len(song_length)), song_length, color=colors, width=0.8, align='center')
    # 添加柱状图边框
    for num, (i, j) in enumerate(zip(range(len(song_length)), song_length)):
        if score_ls[num].rank in ('X', 'XH'):
            rect = plt.Rectangle((i - 0.4, 0), 0.8, j, edgecolor='yellow', facecolor='none', linewidth=2)
            ax1.add_patch(rect)
    # 设置 y 轴刻度为 xx:xx 的格式
    ax2.plot([i.pp for i in score_ls])
    ax1.yaxis.set_major_locator(ticker.MultipleLocator(base=60))  # 设置主刻度为每分钟
    ax1.yaxis.set_major_formatter(ticker.FuncFormatter(lambda x, pos: f"{x // 60:.0f}分钟"))
    plt.show()
    # 统计mods pp
    ...
    legend_labels = [f'{label}: {size:.0f}pp' for label, size in zip(mods_pp.keys(), mods_pp.values())]
