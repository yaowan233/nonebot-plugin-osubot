...
def game_running(event: Union[v11GroupMessageEvent, RedGroupMessageEvent]) -> bool:
    return bool(games.get(int(event.group_id), None))


def pic_game_running(event: Union[v11GroupMessageEvent, RedGroupMessageEvent]) -> bool:
    return bool(pic_games.get(int(event.group_id), None))
...
@word_matcher.handle()
async def _(event: v11GroupMessageEvent):
