...
async def _(event: Union[v11MessageEvent, GuildMessageEvent], state: T_State, args: Message = CommandArg()):
    ...
    if not data:
        await generate_preview.finish(v11MessageSegment.reply(event.message_id) + '未查询到该地图')
    ...
    await generate_preview.finish(v11MessageSegment.reply(event.message_id) + v11MessageSegment.image(pic))
...
async def _(event: RedMessageEvent, state: T_State, args: Message = CommandArg()):
    ...
    if not data:
        await generate_preview.finish(RedMessageSegment.reply(event.msgSeq, event.msgId, event.senderUid) +
                                      '未查询到该地图')
    if isinstance(data, str):
        await generate_preview.finish(RedMessageSegment.reply(event.msgSeq, event.msgId, event.senderUid) + data)
    osu = await download_tmp_osu(osu_id)
    if state['_prefix']['command'][0] == '完整预览':
        pic = await generate_preview_pic(osu, True)
    else:
