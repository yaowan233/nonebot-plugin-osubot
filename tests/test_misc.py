...
@pytest.mark.asyncio
async def test_osu_help_has_handler(app: App):
    """测试 osu_help matcher 有处理器"""
    from nonebot_plugin_osubot.matcher.osu_help import osu_help
    
    assert len(osu_help.handlers) > 0, "osu_help matcher 应该有处理器"
...
@pytest.mark.asyncio
async def test_url_match_has_handler(app: App):
    """测试 url_match matcher 有处理器"""
    from nonebot_plugin_osubot.matcher.url_match import url_match
    
    assert len(url_match.handlers) > 0, "url_match matcher 应该有处理器"
...
@pytest.mark.asyncio
async def test_recommend_has_handler(app: App):
    """测试 recommend matcher 有处理器"""
    from nonebot_plugin_osubot.matcher.recommend import recommend
    
    assert len(recommend.handlers) > 0, "recommend matcher 应该有处理器"
...
@pytest.mark.asyncio
async def test_match_has_handler(app: App):
    """测试 match matcher 有处理器"""
    from nonebot_plugin_osubot.matcher.match import match
    
    assert len(match.handlers) > 0, "match matcher 应该有处理器"
...
@pytest.mark.asyncio
async def test_medal_has_handler(app: App):
    """测试 medal matcher 有处理器"""
    from nonebot_plugin_osubot.matcher.medal import medal
    
    assert len(medal.handlers) > 0, "medal matcher 应该有处理器"
...
@pytest.mark.asyncio
async def test_osudl_has_handler(app: App):
    """测试 osudl matcher 有处理器"""
    from nonebot_plugin_osubot.matcher.osudl import osudl
    
    assert len(osudl.handlers) > 0, "osudl matcher 应该有处理器"
...
@pytest.mark.asyncio
async def test_history_has_handler(app: App):
    """测试 history matcher 有处理器"""
    from nonebot_plugin_osubot.matcher.history import history
    
    assert len(history.handlers) > 0, "history matcher 应该有处理器"
...
@pytest.mark.asyncio
async def test_rating_has_handler(app: App):
    """测试 rating matcher 有处理器"""
    from nonebot_plugin_osubot.matcher.rating import rating
    
    assert len(rating.handlers) > 0, "rating matcher 应该有处理器"
...
@pytest.mark.asyncio
async def test_group_pp_rank_has_handler(app: App):
    """测试 group_pp_rank matcher 有处理器"""
    from nonebot_plugin_osubot.matcher.rank import group_pp_rank
    
    assert len(group_pp_rank.handlers) > 0, "group_pp_rank matcher 应该有处理器"
