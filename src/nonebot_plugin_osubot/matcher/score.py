...
from ..draw import get_score_data, draw_score_history
from ..exceptions import NetworkError

score = on_command("score", aliases={"sc"}, priority=11, block=True)
score_history = on_command(
    "scorelist",
    aliases={"sl", "scorehistory", "历史成绩"},
    priority=11,
    block=True,
)
...
@score_history.handle(parameterless=[split_msg()])
async def _score_history(event: Event, state: T_State):
    if "error" in state:
        await UniMessage.text(state["error"]).finish(reply_to=True)
    explicit_map_id = state["target"]
    map_id = explicit_map_id or get_last_map_id(event)
