...
async def _info(state: T_State):
    ...
    if not server.supports(ServerFeature.PP_HISTORY, server.parse_mode(state["mode"])):
        await UniMessage.text(f"{server.label} 暂不提供 PP/排名历史数据，/hs 仅支持 osu! 官网").finish(reply_to=True)
