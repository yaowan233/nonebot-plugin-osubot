...
from ..api import get_map_scores, get_server, get_user_info_data, osu_api
...
async def draw_score_history(
    ...
    lookup = await get_map_scores(uid, mode, map_json, source, legacy_only=not is_lazer)
    ...
    def build_play(offset: int, score: UnifiedScore) -> dict:
        ...
        return {
            ...
            "score_version": (
                score.score_version if get_server(source).supports(ServerFeature.SCORE_VERSION) else None
            ),
        }
    ...
    data = {
        "source": get_server(source).label,
        ...
        "disclaimer": (
            ...
            if lookup.origin == "official"
            else f"显示 {get_server(source).label} 当前可返回的谱面成绩"
        ),
        "user": {
            "id": info.id,
            "name": info.username,
